...
The MicroBooNE detector is hosted at Fermilab in Batavia, Illinois and is located on-axis to the Booster Neutrino Beam, which delivers a beam of predominantly muon-type neutrinos with energies peaking at 700~MeV. The characteristics of this beamline are well-understood from the work of the MiniBooNE experiment~\cite{AguilarArevalo:2008-MBflux}, which operates directly downstream of the MicroBooNE location. MicroBooNE also records neutrino interactions from an off-axis component of the NuMI beamline~\cite{NuMIbeam}.  Figure \ref{fnalmap} shows the arrangement of MicroBooNE with respect to the BNB and NuMI beamlines at Fermilab.  The physics program of MicroBooNE will utilize both BNB and NuMI samples.  MicroBooNE will also collect data that is out-of-time with either beam, which will be useful for developing analyses (e.g. proton decay searches) relevant for next-generation detectors.  The primary detector technology employed by MicroBooNE is the \lartpc, whose operating principle is described in this section.
...
\caption{Diagram of MicroBooNE LArTPC.  In this view, the beam would be directed out of the page (in the $z$ direction).}
