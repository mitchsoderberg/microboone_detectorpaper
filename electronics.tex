...
After data is written to disk, it is then copied to another server on the internal DAQ network, where the raw data is further compressed, shipped, and queued to be stored on in tape and disk cache using the Fermilab central data management system known as SAM.
%Additionally, this  server runs a ``swizzling" routine to convert the binary data into LArSoft format making it ready to be used for offline analysis. A separate process collects beam data and, during swizzling, inserts that data into the built events. The swizzled data is sent to tape and disk cache on the Fermilab central data management system known as SAM where offline applications can then begin processing the data.
Offline applications then begin processing the raw data in a ``swizzling" routine, converting the binary data format int a LArSoft ROOT-based format which can be used as input for reconstruction algorithms. A separate process collects beam data and, during swizzling, inserts that data into the built events.
