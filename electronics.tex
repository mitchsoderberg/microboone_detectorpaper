...
\begin{center}
\begin{minipage}{12pc}
\includegraphics[width=12pc]{figures/asic.pdf}
\caption{\label{fig:figasic}Layout of the CMOS analog front end ASIC}
\end{minipage}\hspace{1pc}%
\begin{minipage}{20pc}
\includegraphics[width=20pc]{figures/mb2.pdf}
...
The development of the analog front end ASICs was started from 180~nm CMOS technology with only 300 K models while the parameters are extracted at 77 K. CMOS was found to function at cryogenic temperatures with increased gain ($g_{m}/I_{ds}$) and lower noise. The noise, gain, and pulse shaping were found to be as expected in evaluation tests of the ASICs. Extensive test of the ASICs mounted on motherboard was performed in liquid nitrogen given that noise performance can be better evaluated in liquid nitrogen than at room temperature. Thus, cold tests were performed on all production cold motherboards fully populated with 12 chips. A total of $\sim$2,200 chip-immersions were accumulated in liquid nitrogen without any failures due to thermal contraction or expansion. 
...
The \lartpc readout electronics are responsible for processing the signals from the 8,256 wires in MicroBooNE after pre-amplification and shaping in the cold electronics (section~\ref{sec:coldelectronics}). The stages of signal processing are illustrated in figure~\ref{readout_1}. First, the pre-amplified and shaped analog signals from the cold electronics are transmitted to the warm electronics outside the cryostat via a specially designed \lartpc feedthrough, (section~\ref{sec:warmelectronics}). There, the analog signals are further amplified by intermediate amplifiers which are installed in a Faraday cage on top of each feedthrough, thereby making the signals suitable for transmission along 20 m long cables to custom-designed \lartpc readout modules distributed evenly over nine readout crates.  The readout modules digitize the analog signals and then process and prepare them for shipping to designated DAQ machines (one per readout crate) (section~\ref{sec:daq}). 

\begin{figure}
\centering
\includegraphics[width=0.8\linewidth]{./figures/readout_1.jpg}%
\caption{\label{readout_1}\lartpc signal processing and readout stages.}
\end{figure}
...
Separate DRAM multi-event buffers on the FEM store the NU and SN data streams. The data divert into the NU readout stream when a trigger is issued and received (as shown in figure~\ref{fig:readout_4}) signaling, for example, an accelerator neutrino-induced event. When an event trigger is received, 4.8 ms worth of data, relevant to that event, are per-channel packeted and sent to the DAQ through the NU data stream. The 4.8 ms readout size is governed by the drift time and spans three or four frames. In order to reduce the amount of data being transmitted, the FPGA trims the three or four frames to span the exact 4.8 ms required, 1.6 ms before the trigger plus 3.2 ms after the trigger. In parallel, the data is continually sent out through the SN data stream, frame by frame. The compaction and data reduction algorithms applied to each of the two streams are described in the following section.
...
\subsubsection{\lartpc Compression Schemes}
\label{sec:tpccomp}

In the case of the NU data stream, a lossless Huffman coding scheme implemented in the FEM FPGA compacts the data by approximately a factor of five. Further reduction in the overall rate is achieved by exploiting a PMT trigger in coincidence with the BNB trigger, as described in section~\ref{sec:trigger}. Huffman coding provides for lossless data compression by taking advantage of the slow variation of the waveform TPC data in any given channel. In particular, this compression scheme relies on the fact that successive data samples on any given wire vary relatively slowly in time. As such, when noise levels are low, any two adjacent data samples either coincide or differ by 1 ADC count. The most frequent values for the difference in ADCs between successive data samples are assigned pre-specified bit patterns with the lowest number of bits possible. Those bit patterns are compacted in the 16-bit data words that would otherwise be used for a single 12-bit ADC sample value. As such, data reduction of up to a factor of 14 is theoretically possible\footnote{The uppermost two bits are always reserved for header information, in each 16-bit word.}. In practice, the data reduction is sensitive to noise levels and \lartpc activity. In reality, the compression factor achieved by MicroBooNE is roughly a factor of 4-6 as shown in figure~\ref{fig:readout_6}.

\begin{figure}
\centering
\includegraphics[width=0.8\linewidth]{./figures/readout_6.jpg}%
\caption{\label{fig:readout_6}(Placeholder) Compression factors achieved on ADC data with Huffman compression.}
\end{figure}
...
PMT information is recorded in the NU data stream for four 1.6ms frames associated with an event trigger: the frame containing the (asynchronous) trigger, the frame preceding the trigger frame, and two frames following the trigger frame. To avoid the inordinate amount of data that would be generated at a 64 MHz sampling rate, the FEM applies a zero-suppression immediately after digitization, retaining only samples above a given threshold as well as enough information before and after this useful data to establish a local baseline value; this collection of information is referred to as a PMT readout ROI. An exception is formed for BNB and NuMI beam-related data, where the 4$\mu$s-early BNB and NuMI beam gates mentioned in section~\ref{sec:pmtdigit} instruct readout of 1500 consecutive samples (23.4$\mu$s) surrounding and including the beam gates regardless of signal activity.
...
The data from each crate of the backend electronics is sent to a dedicated server (called the sub-event buffer, or SEB) via an optical fiber. The data arrives in a card on the SEB's PCIe bus where a real-time application places data in an internal buffer, collects all of the data belonging to an event, and creates a sub-event fragment that may be routed to a specified destination. For neutrino events, where the data arrives with every trigger, these fragments are sent to a single event-building machine (EVB) over an internal network. Full events are checked for consistency and written to local disk on the EVB before being sent offline for further processing. For supernova stream events, the data remains on the SEB where it is written to disk and only sent for offline analysis on explicit requests. Data writing to either triggered or supernova streams is limited by the RAID6 disk write speeds which are roughly 600 MB/sec. This is less than half of the network bandwidth bottleneck, which is 10 Gbps. The 600 MB/sec disk write speed therefore sets the maximum aggregate rate at which all SEB fragments can ship data to the EVB without loss of data. With Huffman compression, which gives a data reduction of approximately a factor of 4 (figure~\ref{fig:readout_6}) and the PMT trigger, which reduces the data rates by another factor of $>70$, this is more than sufficient for MicroBooNE's maximum 15 Hz beam spill rate. MicroBooNE expects a total triggered write rate of around 4 MB/sec. The supernova circular buffers which will be aggressively (non-losslessly) compressed beyond what the triggered stream experiences will fill each server's RAIDed 14 TB in on the order of a few days, which is ample time to respond to a Super Nova Early Warning System (SNEWS) alert.

After data is written to disk, it is then copied to another server on the internal DAQ network, where the raw data is further compressed, shipped, and queued to be stored on tape. Additionally, this  server runs a ``swizzling" routine to convert the binary data into LArSoft format making it ready to be used for offline analysis. A separate process collects beam data and, during swizzling, inserts that data into the built events. The swizzled data is sent to tape and disk cache on the Fermilab central data management system known as SAM where SAM offline applications can then begin processing the data. A duplicate copy of the triggered swizzled data is also stored offsite at Pacific Northwest National Laboratory (PNNL). This collection of approximately 15 ``projects''  and the database which holds and monitors the state of the data flow is known as the Python/Postgres for MicroBooNE Scripting system (PUBS). As PUBS pushes the data through this process, the progress of each project is closely monitored via a PUBS monitoring GUI. PUBS can also monitor the state of the supernova stream data, held locally on the SEBs. PUBS is patterned after a similar database state machine that the Double Chooz experiment used for data management. A separate offline PUBS instance controls the processing of the swizzled data including applying newly calculated calibration constants as part of data quality management.  Figure \ref{fig:dataflow} schematically depicts the flow of data throughout the MicroBooNE experiment.
...
Additional software components handle the management and monitoring of the main DAQ processes moving the data. A run control application, based on work done for NOvA, issues configuration and state-progressing commands to the SEBs and EVB. Configuration states are stored in a dedicated run configuration database, which allows for the setting and preserving of configuration information for the DAQ, readout, and additional components. This database not only allows for creating the large ($\mathcal{two hundred}$ parameters) intricate DAQ run configuration files, but also enforces certain conditions which must hold for consistency. For example, the configuration that initiates the ASICS charge-injection calibration also dials and captures the settings on the external pulser that drives the calibration signal and assures that ASICS gains and peaking time parameters are enforced and recorded. The configuration that runs the PMT flasher calibration additionally ensures that the charge-injection is only run simultaneously, if desired, in certain allowed modes. 

Another important aspect of the system is monitoring the health of the DAQ. Monitoring of DAQ components is accomplished through Ganglia which monitors basic system states (such as CPU, memory, and network usage) as well as allowing use of custom metrics to monitor the data flow and status of the readout electronics. These metrics are sampled and collected by the EPICS slow monitoring and control processes, which provide archiving and alarms on important quantities. Some examples of Ganglia metrics that are monitored and alarmed in EPICS are the rates of growth of the SEB data buffers, the fragment rates leaving each SEB and their arrival at the EVB.  Figure \ref{fig:ganglia} shows some examples of Ganglia metrics.  
