...
Liquid argon is a very bright scintillator, and collecting the light produced when charged particles travel through the \lartpc is an essential aspect of fully understanding the interactions taking place in the detector.  This section describes the light collection systems which has been developed and implemented in MicroBooNE.
...
However, because of the long window over which an event occurs (about 1.6 ms maximum drift time at 500 V/cm field), there are many accelerated-induced events in which a cosmic ray muon crosses the detector near the time of the spill.  
Utilizing the more accurate $t_0$ from the light collection system and the position of hits in the photodetectors, one can better reject cosmic ray muon tracks.  
The light can also be used to trigger and select specific types of cosmic-ray calibration events (Michel electrons, straight-through muons, etc.) and non-beam events (supernova neutrinos, atmospheric neutrinos, etc.).


\begin{table}[t]
   \centering
    \caption{Important properties of scintilation light in liquid argon that affect detection in MicroBooNE} 
...
The two sources of light in liquid argon are from scintillation and Cherenkov radiation.  
...
The scintillation light has a prompt and slow component with decay times of about 6 ns and 1.6 $\mu$s, respectively. The two lifetimes correspond to the two lowest-lying eximer states with the prompt component coming from the decay of a singlet state and the slow from the decay of a triplet state.  The prompt to slow ratio is about 1:3 for minimum ionizing particles and varies with ionization density argon and particle type. Both components consist of photons with a wavelength of 128 nm.
...
\begin{figure}[t]
    ...
    \caption{Two sequential event displays for the light collection system.  The sequence is consistent with a muon that stops (top) and decays (bottom).  The circles correspond to the PMT units. Red circles indicate those units with hits.    The waveforms versus time are shown below.}\label{fig:muondecaylight}
\end{figure}

The light collection system consists of primary and secondary sub-systems.
The primary light collection system is comprised of PMTs located behind wavelength shifting plates, called ``optical units.''
In total, 32 optical units were installed, yielding 0.85\% photocathode coverage.
The secondary system consists of four light guide paddles.   These paddles are introduced for R\&D studies for future LArTPCs, and are placed near the primary optical units to allow a comparison of their performances.
A flasher system, used for calibration, consists of optical
fibers bringing visible light from an LED to each PMT face.

The light collection detectors are located in the $y$-$z$ plane behind the anode planes of the LArTPC, as shown in figure~\ref{fig:lightlayout}.  The combined transparency of the three anode planes is 86\% for light at normal incidence.  This transparency value assumes 100$\%$ of VUV photons impinging on the wires (150 $\mu$m diameter and 3 mm pitch) are absorbed: $(1-0.150/3)^3 = 86\%$.  The detectors were placed so as not to be obscured by the \lartpc structural cross-bars, shown in figure~\ref{fig:lightlayout}.  Locating the light detectors behind the anode plane places them in a very weak electric field due to the +440 V bias of the collection plane.  To test for an effect from weak electric fields, the response of a PMT placed between a +700 V mesh and ground, separated by 50 cm, was studied  The PMT zenith was 25 cm from the HV.   No effects on the signal were observed.  This was expected, as the photocathode is held at ground, effectively acting as a Faraday cage.  
...
The R5912-02mod employs a bi-alkali photo-cathode.  Because the PMT is designed for cryogenic use, the R5912-02mod also features a thin platinum layer between the photocathode and the borosilicate glass envelope to preserve the conductance at low temperatures.   While this allows the PMT to function below 150 K \cite{HOMeyer}, absorption in the platinum reduces the efficiency of the PMT by 20\%.
...
The result is that the high voltage (HV) and signal can be provided to the PMT using a single cable, 
...
%The material has been tested to not contaminate liqduid argon.
...
at room temperature and in liquid nitrogen (LN2) which is at 77 K.  
...
LN2-filled dewar into which up to four PMTs could be installed. 
...
\item If the room temperature PMTs were immersed in LN2, 
...
\item The HV for each PMT was selected to produce a gain of $3\times 10^7$ in LN2,
and was typically chosen to be $\sim$1300 V.
\item The dark current plateaus extended up to 1800 V in LN2, also the dark current is higher than room temperature.
...
As expected at cryogenic temperatures, the gain is $\sim$10\% to 50\% reduced.
To measure the PMT gain, the LED was run producing 1 to 2 PEs.  
The gain was found from the separation of the single photo-electron peak from the pedestal, where the single photo-electron response is fitted using the  procedure
described in Ref.~\cite{Bellamy:1994bv}.   

Second, it was found that the PMTs are noisier in the cryogenic environment. 
Naively, thermal emission is suppressed at the cold temperature and one can expect lower dark current 
at the low temperature operation of PMT.  
However, the dark current measured in the LN2 is higher than at room temperature. 
...
Third, an LED pulse rate dependent gain shift was found during testing, as shown on figure~9 of Ref.~\cite{Briese:2013wua}.  This behavior is described qualitatively in \cite{HamamatsuBook}.  With 10 kHz LED pulsing, PMT gain of cold and dark-adapted tubes were shown to steadily increase, requiring nearly 24 hours from turn-on to stabilize. The effect was not observable at 10 Hz.   This is relevant to MicroBooNE because the
...
\includegraphics[width=\textwidth]{./light_figures/PlateModeAndPhoto.pdf}
\caption{Left : Illustration of transmission mode, used but the optical units. Right: photograph of a coated plate. \label{fig:PlateCoating}}
\end{figure}
...
The trajectories of electrons within the 8" PMT can be deflected by the Earth's magnetic field.  This effect can be reduced or removed by surrounding PMT with mu metal, a metal of low magnetic permeability.  Commonly used mu-metal fails to provide shielding at cryogenic temperatures. Two types of cryogenic mu metal,Cryoperm 10 and A4K, both products of Amuneal, were identified that did provide shielding at cryogenic temperature. 

The mu metal shields were tested in the apparatus shown in figure~\ref{fig:shieldcartoon}.  The system allowed for the PMT to be positioned at an angle relative to the vertical axis, with the rotator set to 30 positions from 0 to 348$^\circ$.    The set-up was on a dolly that allowed for rotation about the vertical axis.
PMT tests were performed in air and in LN$_2$.   PMTs were dark adapted for 5 hours before testing.  Each data run consisted of a single dolly position and 30 angles from vertical, and took about four hours to complete.  A blue LED provided 1 to 2 single photo-electron of light through an optical fiber.   The fiber was fixed to the PMT mount such that the endpoint was stationary with respect to the tube as the system was rotated.   
...
\subsection{The Bo Vertical Slice Tests}
...
A vertical slice test (VST) of the MicroBooNE optical system was performed in the Bo Cryostat.  The slice consisted of two 8 inch PMTs with base electronics, mu-metal shield, TPB plates,  cable feed throughs, splitters, the HV power supply and the interlock system.   Tests were performed without and with the mu-metal shield.   The test made use of the DAQ components described in section~\ref{sec:electronics}, including the shaper, FEM, trigger card, control card, server.  The MicroBooNE trace impurity monitors were also used.
